...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(top_srcdir)/etc/Makefile.common $(top_srcdir)/etc/compile \
	$(top_srcdir)/etc/config.guess $(top_srcdir)/etc/config.h.in \
	$(top_srcdir)/etc/config.sub $(top_srcdir)/etc/install-sh \
	$(top_srcdir)/etc/missing COPYING COPYING.DOC INSTALL NEWS \
	README THANKS etc/compile etc/config.guess etc/config.sub \
	etc/depcomp etc/install-sh etc/missing etc/ylwrap
...
YACC = bison -y
YFLAGS = 
abs_builddir = /Users/tonietto/Downloads/ifm-5.5
abs_srcdir = /Users/tonietto/Downloads/ifm-5.5
...
etc/config.h: etc/stamp-h1
	@test -f $@ || rm -f etc/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) etc/stamp-h1

etc/stamp-h1: $(top_srcdir)/etc/config.h.in $(top_builddir)/config.status
	@rm -f etc/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status etc/config.h
$(top_srcdir)/etc/config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f etc/stamp-h1
	touch $@

distclean-hdr:
	-rm -f etc/config.h etc/stamp-h1
